...
public:
  ...
  struct uniform_t {
    ...
    explicit uniform_t(int location) : location(location) {}
  };
  ...
  std::variant<uniform_t, std::string> get_uniform(
      const std::string& uniform_id
  ) {
    if (shader_program_handler == oo) {
      return "Trying to get a uniform from a non-initialized shader_program ["
          + shader_id + "]. Did you forget to invoke load()?";
    }
    ...
    return uniform_t{ loc };
  }

  void set_uniform_value_4f( //
      const uniform_t& uniform, //
      const float v1, //
      const float v2, //
      const float v3, //
      const float v4 //
  ) {
    glUniform4f(uniform.location, v1, v2, v3, v4);
  }
};